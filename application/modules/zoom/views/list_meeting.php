		...
		<div class="col-lg-12">
			<div class="card m-b-30">
				...
				<div class="tabset">
						  ...
						  <label for="tab7" ><i class="feather icon-video mr-2"></i><?php  echo lang('live'); ?> <?php  echo lang('meetings'); ?></label>
						  <!-- Tab 2 -->
						  <input type="radio" name="tabset" id="tab8" aria-controls="Olawuyi">
						  <label for="tab8"> <i class="feather icon-video-off mr-2"></i><?php  echo lang('archived'); ?> <?php  echo lang('meetings'); ?></label>
									...
									<table class="table table-striped table-hover table-bordered" id="bateristaworks">
										<thead>
											<tr>                      
												...
												<th><?php echo 'Start Date - Time'; ?></th>
												<th><?php echo 'Meeting Token'; ?></th>
										...
										<tbody>
											<?php 
												if ($this->ion_auth->in_group('Patient')) { 
													$patient_ion_idd = $this->ion_auth->get_user_id();
													$patient_idd = $this->db->get_where('patient', array('ion_user_id' => $patient_ion_idd))->row()->id;
												 } 
											 ?>
											 
											<?php 
												if ($this->ion_auth->in_group('Staff')) { 
													$doctor_ion_idd = $this->ion_auth->get_user_id();
													$doctor_idd = $this->db->get_where('staff', array('ion_user_id' => $doctor_ion_idd))->row()->id;
												} 
											?>
											
											<?php foreach ($meetings as $meeting) { 
											if($meeting->patient === $patient_idd && round(abs($meeting->registration_time - time()) / 60,2) < 32 || $meeting->doctor === $doctor_idd && round(abs($meeting->registration_time - time()) / 60,2) < 32){ ?>
												<tr class="">
													<td><?php echo $this->db->get_where('patient', array('id' => $meeting->patient))->row()->name; ?></td>
													<td><?php echo $this->db->get_where('staff', array('id' => $meeting->doctor))->row()->name; ?></td>
													<td><?php  echo date('d-m-Y - H:i A', $meeting->registration_time); ?></td>
													<td><?php  echo $meeting->meeting_id; ?></td>
													<td>
														<a class="btn btn-info btn-xs btn_width zoom_color" href="zoom/zoomMeeting?zoom_meeting_soft=<?php echo $meeting->id; ?>&zoom_meeting_token=<?php echo $meeting->meeting_id; ?>"  target="_blanc" ><i class="feather icon-video"></i> <?php echo 'Join Meeting'; ?></a>	
													</td>
												</tr>
											<?php } ?>
											<?php } ?>
