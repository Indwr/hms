		...
		<div class="col-lg-12">
			<div class="card m-b-30">
				...
				<div class="tabset">
						  ...
						  <label for="tab7" ><i class="fa fa-medkit mr-2"></i><?php  echo lang('all'); ?> <?php  echo lang('medicine'); ?></label>
						  <!-- Tab 2 -->
						  <input type="radio" name="tabset" id="tab8" aria-controls="Olawuyi">
						  <label for="tab8"> <i class="fa fa-medkit mr-2"></i><?php  echo lang('soon_expire'); ?> <?php  echo lang('medicine'); ?> 
						  &nbsp;
						   <?php 
							$query_medicine = $this->db->get('payment_category')->result();
							$i = 0;
							foreach ($query_medicine as $q_medicine) {
								if(!empty($q_medicine->e_date) && strtotime($q_medicine->e_date) <= strtotime("+2 month") && (strtotime($q_medicine->e_date) > time())  &&  ($q_medicine->howmany) > 0 && empty($q_medicine->deletii)) {
							     $i = $i + 1;
								}
							}
							if (!empty($i)) { ?>
						  ...
						  <label for="tab9"><i class=" fa fa-medkit mr-2"></i><?php  echo lang('expired'); ?> <?php  echo lang('medicine'); ?> 
						   &nbsp;
						   <?php 
							$query_medicine = $this->db->get('payment_category')->result();
							$i = 0;
							foreach ($query_medicine as $q_medicine) {
								if(!empty($q_medicine->e_date) && strtotime($q_medicine->e_date) <= time() &&  ($q_medicine->howmany) > 0 && empty($q_medicine->deletii)){
							     $i = $i + 1;
								}
							}
							if (!empty($i)) { ?>
							<span class="badge badge-danger pull-right"><?php echo $i; ?></span>
							<?php } else{?>
							<span class="badge badge-danger pull-right"><?php echo $i; ?></span>
							<?php } ?>
						  </label>
						  
						  <input type="radio" name="tabset" id="tab10" aria-controls="Baterista2">
						  <label for="tab10"><i class=" fa fa-medkit mr-2"></i><?php  echo lang('out_of_stock'); ?> <?php  echo lang('medicine'); ?> 
						   &nbsp;
						   <?php 
							$query_medicine = $this->db->get('payment_category')->result();
							$i = 0;
										...
										<a data-toggle="modal" href="#myModal">
											<div class="btn-group">
												<button id="" class="btn green">
													<i class="fa fa-plus-circle"></i>  <?php  echo lang('add'); ?>  <?php  echo lang('medicine'); ?>
												</button>
											</div>
										</a>
									 </div>
						...
						if(!empty($medicine->e_date) && empty($medicine->deletii)){?>
                            <tr class="">
								...
								<?php echo $medicine->e_date; ?>
                                    ...
                                    <td>
                                       <button type="button" class="btn btn-info btn-xs btn_width editbutton" data-toggle="modal" data-id="<?php echo $medicine->id; ?>"><i class="fa fa-edit"></i> Edit</button>   
                                        <a class="btn btn-info btn-xs delete_button" href="pharmacy/deleteMedicine?id=<?php echo $medicine->id; ?>" onclick="return confirm('Are you sure you want to delete this medicine?');"><i class="fa fa-trash-o"></i></a>
                                    </td>
                        ...
                        <?php foreach ($medicines as $medicine) { 
						if(!empty($q_medicine->e_date) && strtotime($medicine->e_date) <= strtotime("+2 month") && (strtotime($medicine->e_date) > time())  &&  ($medicine->howmany) > 0 && empty($medicine->deletii)) { ?>
                            <tr class="">
                        ...
                        <?php foreach ($medicines as $medicine) { 
						if(!empty($q_medicine->e_date) && strtotime($medicine->e_date) <= time() &&  ($medicine->howmany) > 0 && empty($medicine->deletii)) {?>
                            <tr class="">
                        ...
                        <?php foreach ($medicines as $medicine) { 
						if(!empty($medicine->e_date) && $medicine->e_date <= time() &&  ($medicine->howmany) == 0 && empty($medicine->deletii)) {?>
                            <tr class="">
